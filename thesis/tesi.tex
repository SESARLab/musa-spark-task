...
Il codice python del modello è stato inserito direttamente nell'immagine \gls{docker}, caricata su Docker Hub, in questo modo ogni Pod del \gls{cluster} può fare il pull dell'immagine ed eseguire il codice.
...
\begin{lstlisting}
I tensorflow/core/distributed_runtime/rpc/grpc_server_lib.cc:449] Started server with target: grpc://worker0:1999
I tensorflow/tsl/distributed_runtime/coordination/coordination_service.cc:535] /job:worker/replica:0/task:0 has connected to coordination service. Incarnation: 15228123928711275858
...
\end{lstlisting}
La prima riga indica che un server gRPC è stato avviato sul nodo worker0 sulla porta 1999. Questo server verrà utilizzato per la comunicazione tra i nodi worker nella configurazione distribuita. Il prefisso \lstinline{grpc://} indica che il server utilizza il protocollo gRPC per la comunicazione.
...
L'ultima riga indica che l'agente di coordinamento, responsabile della comunicazione con il servizio di coordinamento, si è connesso correttamente al servizio. Ciò significa che il nodo worker0 è ora pronto per partecipare al processo di training distribuito.
