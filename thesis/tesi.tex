...
\chapter*{Prefazione}
\pagestyle{plain}
A partire dai primi anni 2000 si sono sviluppate numerose tecnologie per l'elaborazione distribuita di \bd. Il grande numero di tecnologie differenti ha fatto sì che venissero sviluppati software strettamente legati all'infrastruttura di esecuzione. In questa tesi si vuole analizzare l'esecuzione su un'unica infrastruttura algoritmi che utilizzano tecnologie diverse. L'elaborato si concentra sulla possibilità di utilizzare —con modifiche minime o assenti— codice non pensato originariamente per l'esecuzione distribuita su un \gls{cluster} \gls{kube}.
...
\chapter{Conclusioni}\label{ch:conc}
    ...
    Questa tesi ha fornito, in aggiunta, una panoramica sull'integrazione delle tecnologie Spark, Kubernetes e Tensorflow, aprendo la possibilità di sviluppare ambienti distribuiti per il trattamento di \bd che utilizzano i diversi metodi di elaborazione.
    Infine, il recupero degli algoritmi \gls{ml} sviluppati come Spark tasks in linguaggio Java, insieme all'implementazione di nuovi algoritmi utilizzando Python e TensorFlow, ha dimostrato chiaramente i vantaggi derivanti dalla separazione del codice dall'infrastruttura di esecuzione.
